$M.notice.edit = function (S) {
    ...
    var read = function (dataId) {
        if (dataId) {
            $M.comm("notice.ajax.read", { id: dataId }, function (json) {
                form.val(json);
            });
        }
    };
...
$M.notice.messageList = function (S) {
    ...
    var reload = function (pageno) {
        ...
        $M.comm("notice.ajax.messageList", { pageNo: pageno ,"status":_status}, function (json) {
            userList.clear();
        ...
        xtype: "ToolBar", items: [
        {
            ...
            text: "删除", onClick: function () {
                ...
                $M.confirm("您确定要删除所选消息吗？", function () {
                    var ids = getId();
                    if (ids != "") {
                        $M.comm("notice.ajax.delMessage", { ids: ids }, reload);
                    }
                });
            }
        }]
...
$M.notice.send = function (S) {
    ...
    var read = function (dataId) {
        if (dataId) {
            $M.comm([["notice.ajax.read", { id: dataId }], ["notice.ajax.setMessageStatus", { id: dataId,status:1 }]], function (json) {
