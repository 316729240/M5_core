$M.questionManage.del = function (S) {
    $M.comm("questionManage.ajax.delData", { ids: S.ids, moduleId: S.moduleId, classId: S.classId, tag: S.type == 0 ? 0 : 1 }, function (json) {
        S.back();
    });
};
$M.questionManage.addAnswer = function (S) {
    var obj = $M.dialog({
        title:"编辑回答",
        style: { width: "560px" },
        command: "questionManage.ajax.editAnswer",
    ...
    if (S.answerId) {
        $M.comm("questionManage.ajax.readAnswer", { id: S.answerId }, function (json) {
...
$M.questionManage.edit = function (S) {
    ...
    var form = tab2.items[0].addControl({
        xtype: "Form",
        command: "questionManage.ajax.edit",
    ...
    var read = function (dataId) {
        if (dataId) {
            $M.comm("questionManage.ajax.read", { id: dataId }, function (json) {
                form.val(json);
            });
        }
    };
    ...
    var setAnswerId = function () {
        ...
        $M.comm("questionManage.ajax.setAnswerId", { id: dataId.val(), answerId: grid.selectedRows[0].cells[0].val() }, function () {
    ...
    var delData = function () {
        ...
        $M.confirm("您确定要删除所选数据吗？", function () {
            $M.comm("questionManage.ajax.delAnswer", { dataId: dataId.val(), ids: ids }, function () {
                reload();
            });
        });
    };
    ...
    var reload = function () {
        $M.comm("questionManage.ajax.list", { pageNo: 1,dataId:S.dataId }, function (json) {
            grid.clear();
            grid.addRow(json);
        });
    };
