$M.dataBase.reduction = function (S) {
    ...
    var reductionData = function () {
        ...
        $M.comm("dataBase.ajax.readTableData", {  backName: S.name,tableName: data[index].tableName, pageSize: data[index].pageSize, pageNo: pageNo }, function (json) {
            p2.val(pageNo);
    ...
    var zipData = function () {
        label1.html("正在执行脚本...");
        $M.comm("dataBase.ajax.runScript", { backName: S.name }, function (json) {
            isClose = true;
            win.remove();
        });
    };
    $M.comm("dataBase.ajax.readTableStructure", { backName: S.name }, function (json) {
        data = json;
