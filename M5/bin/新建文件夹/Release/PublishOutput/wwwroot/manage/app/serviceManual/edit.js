...
$M.serviceManual.edit = function (S) {
    ...
    var form = frame.items[0].addControl({
        xtype: "Form",
        command: "serviceManual.ajax.edit",
    ...
    var loadSelectItem = function (t, value) {
        ...
        $M.comm("serviceManual.ajax.pinpai", v, function (json) {
            obj.clear();
    ...
    var read = function (dataId) {
        if (dataId) {
            $M.comm("serviceManual.ajax.read", { id: dataId }, function (json) {
                setForm(json);
            });
        }
    };
    ...
    if (S.dataId != null) {
        $M.comm("serviceManual.ajax.read", { id: S.dataId }, function (data) {


            setForm(data);
            $M.comm([
            ["serviceManual.ajax.pinpai", { t: 0 }],
            ["serviceManual.ajax.pinpai", { t: 1, a1: data.carInfo[0] }],
            ["serviceManual.ajax.pinpai", { t: 2, a1: data.carInfo[0], a2: data.carInfo[1] }],
            ["serviceManual.ajax.pinpai", { t: 3, a1: data.carInfo[0], a2: data.carInfo[1], a3: data.carInfo[2] }],
            ["serviceManual.ajax.pinpai", { t: 4, a1: data.carInfo[0], a2: data.carInfo[1], a3: data.carInfo[2], a4: data.carInfo[3] }],
            ["serviceManual.ajax.pinpai", { t: 5, a1: data.carInfo[0], a2: data.carInfo[1], a3: data.carInfo[2], a4: data.carInfo[3], a5: data.carInfo[4] }]
            ], function (json) {
                changeFlag = true;
...
$M.serviceManual.addCar = function (S) {
    var tab = $M.dialog({
        title: "编辑车型",
        style: { width: "660px" },
        command: "serviceManual.ajax.editBind",
    ...
    if (S.dataId != null) {
        $M.comm("serviceManual.ajax.readBind", { id: S.dataId }, function (data) {
            setForm(data);
            $M.comm([
            ["serviceManual.ajax.pinpai", { t: 0 }],
            ["serviceManual.ajax.pinpai", { t: 1, a1: data.carInfo[0] }],
...
$M.serviceManual.buyCar = function (S) {
    var tab = $M.dialog({
        title: "添加车型",
        style: { width: "660px" },
        command: "serviceManual.ajax.buyP",
        onClose: function (sender, e) {
            if (sender.dialogResult == $M.dialogResult.ok) {
    ...
    $M.comm("serviceManual.ajax.readP1", { userId:S.userId,t: 0 }, function (json) {
                selects[0].addItem(json);
    });
    selects[0].attr("onChange", function (sender, e) {
        $M.comm("serviceManual.ajax.readP1", { userId: S.userId, t: 1, a1: sender.val() }, function (json) {
            selects[1].clear();
            selects[1].addItem({ text: "请选择", value: "" });
            selects[1].addItem(json);
        });
        $M.comm("serviceManual.ajax.readP2", { t: 1, a1: sender.val() }, function (json) {
            grid.clear();
            grid.addRow(json);
        });
    });
    selects[1].attr("onChange", function (sender, e) {
        $M.comm("serviceManual.ajax.readP1", { userId: S.userId, t: 2, a1: selects[0].val(), a2: selects[1].val() }, function (json) {
            selects[2].clear();
            selects[2].addItem({ text: "请选择", value: "" });
            selects[2].addItem(json);
        });
        $M.comm("serviceManual.ajax.readP2", { t: 2, a1: selects[0].val(), a2: selects[1].val() }, function (json) {
            grid.clear();
            grid.addRow(json);
        });
    });
    ...
    selects[2].attr("onChange", function (sender, e) {
        $M.comm("serviceManual.ajax.readP2", { t: 3, a1: selects[0].val(), a2: selects[1].val(), a3: selects[2].val() }, function (json) {
            grid.clear();
            grid.addRow(json);
        });
    });
