...
public class ajax : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (m == "read")
        {
            ...
            Dictionary<string, object> data = Helper.Sql.ExecuteDictionary("select A.title,A.classId,A.skinId,A.url,A.pic,B.* from mainTable A inner join  u_xueyuanxinxi B on A.id=B.id where A.id=@id", new SqlParameter[] { new SqlParameter("id", id) });
        ...
        {
            ErrInfo info = new ErrInfo();
            RecordClass value = new RecordClass(22192428232,login.value);
            double id=s_request.getDouble("id");
            ...
            TableInfo table = new TableInfo(22192428232);
