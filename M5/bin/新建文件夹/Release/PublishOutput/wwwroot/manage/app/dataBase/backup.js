$M.dataBase.backup = function (S) {
    ...
    var saveData = function () {
        ...
        $M.comm("dataBase.ajax.saveTableData", {backName:S.name, tableName: data[index].tableName, pageSize: data[index].pageSize, pageNo: pageNo }, function (json) {
            p2.val(pageNo);
    ...
    var zipData = function () {
        label1.html("正在打包数据...");
        $M.comm("dataBase.ajax.zipData", { backName: S.name }, function (json) {
            isClose = true;
            win.remove();
        });
    };
    $M.comm("dataBase.ajax.saveTableStructure", { backName: S.name }, function (json) {
        data = json;
