$M.account.edit = function (S) {
    ...
    if (S.classId == "9896847028") {
        ...
        shenheButton.find("button").click(function () {
            $M.comm("account.ajax.shenhezizhi", { id: S.dataId, type: $(this).attr("val") }, function () {
                S.back();
                alert("操作成功");
            });
            return false;
    ...
    var read = function (dataId) {
        if (dataId) {
            $M.comm("account.ajax.read", { id: dataId }, function (json) {
    ...
    var delData = function () {
        ...
        $M.confirm("您确定要删除所选数据吗？", function () {
            $M.comm("serviceManual.ajax.delCar", {ids: ids }, function () {
                reload();
            });
        });
    };
    var delData2 = function () {
        ...
        if (ids == "") return;
        $M.confirm("您确定要删除所选数据吗？", function () {
            $M.comm("serviceManual.ajax.delBuyCar", { ids: ids }, function () {
                reload2();
            });
        });
    };
    ...
    var reload = function () {
        $M.comm("serviceManual.ajax.carlist", { pageNo: 1, dataId: S.dataId }, function (json) {
            grid.clear();
            grid.addRow(json);
        });
    };
    var reload2 = function () {
        $M.comm("serviceManual.ajax.buyList", { pageNo: 1, userId: S.dataId }, function (json) {
            grid2.clear();
            grid2.addRow(json);
        });
    };
