        ...
        onRowCommand: function (sender, e) {
            if (e.x == 5) {
                if (grid.rows[e.y].cells[5].attr("foreColor") ==null) {
                    ...
                    $M.comm("appShop.ajax.setup", { appName: grid.rows[e.y].cells[5].val(), datetime: grid.rows[e.y].cells[6].val() }, function () {
                ...
                $M.confirm("您确定要删除该应用吗？", function () {
                    $M.comm("appShop.ajax.uninstall", { appName: grid.rows[e.y].cells[5].val() }, function () {
